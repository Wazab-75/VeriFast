...
from julia_generator import generate_julia_image  # You must create this file/function
import cmath

app = Flask(__name__)
performance_log = []  # Store performance data
...
def generate_mandelbrot():
    try:
        ...
        if width <= 0 or height <= 0 or max_iter <= 0 or zoom <= 0:
            return "Invalid dimensions, iterations or zoom", 400

        buf, elapsed = generate_image(width, height, max_iter, zoom, cx, cy, cmap)
        performance_log.append({'type': 'mandelbrot', 'resolution': f'{width}x{height}', 'time': round(elapsed, 3)})

        return send_file(buf, mimetype='image/png')

    except Exception as e:
        print(f"Error generating Mandelbrot: {str(e)}")
        return str(e), 500
...
def generate_julia():
    try:
        ...
        max_iter = int(data['max_iter'])
        zoom = float(data['zoom'])
        zx = float(data.get('zx', 0.0))  # Center view for Julia
        zy = float(data.get('zy', 0.0))
        c_real = float(data['center_x'])  # Used as 'c' for Julia
        ...
        performance_log.append({'type': 'julia', 'resolution': f'{width}x{height}', 'time': round(elapsed, 3)})

        return send_file(buf, mimetype='image/png')

    except Exception as e:
        print(f"Error generating Julia: {str(e)}")
        return str(e), 500
